...
func (s *ProjectService) publishProjectEvent(
	...
	projectID, actorID, userID string,
	now time.Time,
	...
	eventID, err := s.events.AppendDomainEvent(ctx, eventType, "project", projectID, actorID, userID, payloadJSON)
	if err != nil {
		return err
	}
...
func (s *ProjectService) Create(ctx context.Context, userID, title, actorID string, opts ...string) (*domain.Project, error) {
	if title == "" {
	...
	if err := s.publishProjectEvent(ctx, domain.ProjectCreated, project.ID, actorID, userID, now, payload, domain.DeltaCreated, nil, nil); err != nil {
		return nil, err
	}
...
func (s *ProjectService) hydrateTags(ctx context.Context, userID string, project *domain.Project) error {
	tags, err := s.queries.ListProjectTags(ctx, sqlc.ListProjectTagsParams{ProjectID: project.ID, UserID: userID})
	if err != nil {
		return err
	}
...
func (s *ProjectService) Get(ctx context.Context, userID, id string) (*domain.Project, error) {
	row, err := s.queries.GetProject(ctx, sqlc.GetProjectParams{ID: id, UserID: userID})
	if err != nil {
		return nil, err
	}
	project := projectFromRow(row)
	if err := s.hydrateTags(ctx, userID, project); err != nil {
		return nil, err
	}
	return project, nil
}
...
func (s *ProjectService) Complete(ctx context.Context, userID, id, actorID string) error {
	now := timeNow()
	...
	payload := map[string]any{}
	return s.publishProjectEvent(ctx, domain.ProjectCompleted, id, actorID, userID, now, payload, domain.DeltaModified, strPtr("status"), json.RawMessage(`"completed"`))
}

// Cancel sets the project status to cancelled, cascades to mark all pending tasks
// as cancelled, and emits project.cancelled.
func (s *ProjectService) Cancel(ctx context.Context, userID, id, actorID string) error {
	now := timeNow()
	...
	payload := map[string]any{}
	return s.publishProjectEvent(ctx, domain.ProjectCancelled, id, actorID, userID, now, payload, domain.DeltaModified, strPtr("status"), json.RawMessage(`"cancelled"`))
}

// UpdateTitle validates and updates the project title, then emits project.title_changed.
func (s *ProjectService) UpdateTitle(ctx context.Context, userID, id, title, actorID string) error {
	if title == "" {
	...
	return s.publishProjectEvent(ctx, domain.ProjectTitleChanged, id, actorID, userID, now, payload, domain.DeltaModified, strPtr("title"), titleJSON)
}

// UpdateNotes updates the project notes and emits project.notes_changed.
func (s *ProjectService) UpdateNotes(ctx context.Context, userID, id, notes, actorID string) error {
	now := timeNow()
	...
	return s.publishProjectEvent(ctx, domain.ProjectNotesChanged, id, actorID, userID, now, payload, domain.DeltaModified, strPtr("notes"), notesJSON)
}

// SetDeadline sets the project deadline and emits project.deadline_set or project.deadline_removed if nil.
func (s *ProjectService) SetDeadline(ctx context.Context, userID, id string, date *time.Time, actorID string) error {
	now := timeNow()
	...
	return s.publishProjectEvent(ctx, eventType, id, actorID, userID, now, payload, domain.DeltaModified, strPtr("deadline"), nil)
}
...
func (s *ProjectService) MoveToArea(ctx context.Context, userID, id string, areaID *string, actorID string) error {
	now := timeNow()
	...
	return s.publishProjectEvent(ctx, eventType, id, actorID, userID, now, payload, domain.DeltaModified, strPtr("area_id"), nil)
}

// AddTag adds a tag to the project and emits project.tag_added. Verifies the tag belongs
// to userID (spec §2.4) before mutating, returning domain.ErrNotFound if it does not.
func (s *ProjectService) AddTag(ctx context.Context, userID, id, tagID, actorID string) error {
	now := timeNow()
	...
	return s.publishProjectEvent(ctx, domain.ProjectTagAdded, id, actorID, userID, now, payload, domain.DeltaModified, strPtr("tags"), nil)
}

// RemoveTag removes a tag from the project and emits project.tag_removed.
func (s *ProjectService) RemoveTag(ctx context.Context, userID, id, tagID, actorID string) error {
	now := timeNow()
	...
	payload := map[string]any{"tag_id": tagID}
	return s.publishProjectEvent(ctx, domain.ProjectTagRemoved, id, actorID, userID, now, payload, domain.DeltaModified, strPtr("tags"), nil)
}

// UpdateColor updates the project color and emits project.color_changed.
func (s *ProjectService) UpdateColor(ctx context.Context, userID, id, color, actorID string) error {
	now := timeNow()
	...
	return s.publishProjectEvent(ctx, domain.ProjectColorChanged, id, actorID, userID, now, payload, domain.DeltaModified, strPtr("color"), colorJSON)
}
...
func (s *ProjectService) Delete(ctx context.Context, userID, id, actorID string) error {
	now := timeNow()
	...
	payload := map[string]any{}
	return s.publishProjectEvent(ctx, domain.ProjectDeleted, id, actorID, userID, now, payload, domain.DeltaDeleted, nil, nil)
}
