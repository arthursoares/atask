...
|------|-----------------|----------|--------|
| Inbox | `GET /views/inbox` | `created_at DESC` | `schedule = inbox` |
...
- `task.completed` → move from current view to logbook, decrement badge, trigger checkPop animation
...
Response envelope: `{ "data": ..., "event": "task.completed" }`
