...
async fn test_reopen_task() {
    ...
    // Should be back in inbox (status=pending, schedule=inbox)
    let inbox = client.list_inbox().await.unwrap();
    assert!(inbox.iter().any(|t| t.id == task.id));
    client.delete_task(&task.id).await.unwrap();
}
...
#[tokio::test]
async fn test_section_reorder() {
    let client = setup_client().await;
    let project = client.create_project("Section reorder test").await.unwrap();
    let section = client.create_section(&project.id, "Section A").await.unwrap();
    client.reorder_section(&project.id, &section.id, 5).await.unwrap();
    // Verify section still exists
    let sections = client.list_sections(&project.id).await.unwrap();
...
#[tokio::test]
async fn test_tag_on_task() {
    let client = setup_client().await;
    let task = client.create_task("Tag test").await.unwrap();
    let tag = client.create_tag("test-tag").await.unwrap();
    client.add_task_tag(&task.id, &tag.id).await.unwrap();
    // Verify by fetching task (tags are hydrated on GET by ID)
    // Note: list endpoints don't hydrate tags
    client.remove_task_tag(&task.id, &tag.id).await.unwrap();
    client.delete_task(&task.id).await.unwrap();
}
