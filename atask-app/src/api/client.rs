...
impl ApiClient {
    ...
    pub async fn reopen_task(&self, id: &str) -> Result<(), ApiError> {
        self.post_action(&format!("/tasks/{id}/reopen")).await
    }
    ...
    pub async fn update_project_color(&self, id: &str, color: &str) -> Result<(), ApiError> {
        self.put_json(
            &format!("/projects/{id}/color"),
    ...
    pub async fn reorder_section(
        ...
        self.put_json(
            &format!("/projects/{project_id}/sections/{section_id}/reorder"),
            &serde_json::json!({"index": index}),
        )
        .await
    }
